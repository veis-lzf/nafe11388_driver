...
status_t NAFE_HAL_writeRegBlock(uint32_t cmd, uint32_t data, uint32_t dataBits)
{
    ...
    bsp_spi_transmit_byte(SPI_Seq2, ((cmd >> 8) & 0xff));
    bsp_spi_transmit_byte(SPI_Seq2, (cmd & 0xff));
    // 发送DATA[23..0]
    do
    {
        bsp_spi_transmit_byte(SPI_Seq2, ((data >> (dataBits - INDEX_STEP))  & 0xff));
        dataBits -= INDEX_STEP;
        index += INDEX_STEP;
    ...
    return kStatus_Success; // 去掉状态管理去掉，直接返回kStatus_Success
}
...
{
	status_t statusCheck = kStatus_Success;
    uint16_t index = 0;
    ...
    *data = 0;
    do
    {
        (*data) |= bsp_spi_received_byte(SPI_Seq2);
        (*data) <<= INDEX_STEP;
        index += INDEX_STEP;
    } while (index < dataBits);
...
status_t NAFE_startSample(NAFE_devHdl_t *devHdl, NAFE_xferHdl_t *xferHdl)
{
	...
	assert_platfrom(xferHdl->chnAmt <= 16u);
    ...
    switch(xferHdl->sampleMode)
    {
        case kNafeSampleMode_scsrBlock:
            if (devHdl->currentChnIndex != xferHdl->requestedChn)
            {
                ...
                assert_platfrom(devHdl->sysConfig->enabledChnMask & (1 << xferHdl->requestedChn));
...
static void NAFE_combCmdForAccessRegs(uint16_t devAddr, uint16_t regAddr,
                            NAFE_regAccessType_t accessType, uint32_t *combCmd)
{
    assert_platfrom(devAddr == 0u || devAddr == 1u);
                                      ...
                                      uint32_t data, NAFE_regDataSize_t regSize)
{
    ...
    return NAFE_HAL_writeRegBlock(combCmd, data, regSize);
}
                                     ...
                                     uint32_t *data, NAFE_regDataSize_t regSize)
{
    ...
    return NAFE_HAL_readRegBlock(combCmd, data, regSize);
}
...
static inline status_t NAFE_sendCmdBlock(NAFE_devHdl_t *devHdl, uint32_t cmd)
{
    ...
    return NAFE_HAL_sendCmdBlock(combCmd);
}
