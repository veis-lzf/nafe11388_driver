...
#ifndef __BSP_NAFE11388_H__
#define __BSP_NAFE11388_H__
...
#define NAFE_CMD_SS 0x2000 // Set Single-Channel Single-Reading (conversion) mode.
#define NAFE_CMD_SC 0x2001 // Set Single-Channel Continuous-Reading (conversion) mode.
#define NAFE_CMD_MM 0x2002 // with infinite loop until it is interrupted or restarted.
#define NAFE_CMD_MC 0x2003 // Set Multichannel Continuous-readings (conversions) autonomous mode.
#define NAFE_CMD_MS 0x2004 // Set Multichannel Single Reading (conversions) mode.
#define NAFE_CMD_BURST_DATA 0x2005 // Burst read the enabled data channels CH_DATA from CH0 to CH15 sequentially(determined by MCH_EN[15:0] bits).
#define NAFE_CMD_CALC_CRC_CONFIG 0x2006 // Calculate CRC sum for all 16-bit user configuration registers and save result to 0x25\h(CRC_CONFIG_REGS).
#define NAFE_CMD_CALC_CRC_COEF 0x2007 // Calculate CRC sum for all 24-bit CAL coefficient and threshold registers and save result to 0x26\h(CRC_COEF_REGS).
#define NAFE_CMD_CALC_CRC_FAC 0x2008 // Calculate all factory OTP trim registers and save result to 0x27\h(CRC_TRIM_INT[15:0]).
...
#define NAFE_REG_CH_CONFIG4 0x24    /*Enable logical configurable channel for ADC conversion in Sequencer mode: 0 = disable, 1 = enable.CH15 is bit15, CH0 is bit0.*/
